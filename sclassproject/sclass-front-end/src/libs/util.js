...
export const funReviver = (key, value) => {
  if ('string' == typeof value && value.indexOf('function') === 0) {
    var argumentsList = value
      .slice(value.indexOf("(") + 1, value.indexOf(")"))
      .split(",");
    var funcBody = value.slice(
      value.indexOf("{") + 1,
      value.lastIndexOf("}")
    );
    return Function(argumentsList, funcBody);
  }
  return value;
}

export const parseFuncObj = (objStr) => {
  return JSON.parse(objStr, funReviver)
}

export const disableFormItems = (options) => {
  options = (JSON.parse(options));
  options = options.options;
  options.formItem = options.formItem.map((v, k) => {
